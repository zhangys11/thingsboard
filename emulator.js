...
const thingsboardHost = "120.92.150.103";
...
client.on('connect', function () {
    ...
    console.log('Uploading sensor data once per second...');
    publishImage()
    setInterval(publishTelemetry, 1000);
    // setInterval(publishImage, 10000);
});
...
function publishTelemetry() {
    ...
    data.rain = genNextValue(data.humidity, minHumidity, maxHumidity);
...
function publishImage(){    
    data.img = fs.readFileSync('panda.jpg', {encoding: 'base64'});
